...
import reactive_mongo_driver.Good;
...
public class RxNettyHttpServer {
                ...
                .start((req, resp) -> {
                    ...
                    switch (name) {
                        case "createUser":
                            ...
                            break;
                        case "createGood":
                            response = createGood(queryParam);
                            resp.setStatus(HttpResponseStatus.OK);
                            break;
                        case "getGoods":
    ...
    public static Observable<String> createGood(Map<String, List<String>> queryParam) {
        ArrayList<String> required = new ArrayList<>(Arrays.asList("id", "name", "rub", "usd", "eur"));
        ...
        if (ReactiveMongoDriver.createGood(new Good(id, name, rub, usd, eur)) == Success.SUCCESS) {
    ...
    public static Observable<String> getGoods(Map<String, List<String>> queryParam) {
        ...
        Integer id = new Integer(queryParam.get("id").get(0));
        Observable<String> goods = ReactiveMongoDriver.getAllGoods(id);
        return Observable.just("{ user_id = " + id + ", goods = [").concatWith(goods).concatWith(Observable.just("]}"));
    }
